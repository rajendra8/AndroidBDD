...
public class ActionsClass extends Base {
    ...
    public static boolean click(String str_Element) {
        try {
            ...
            fetchElement(str_Arr).isDisplayed();
        ...
        try {
            ...
            if (fetchElements(str_Arr).size() > 0) {
                if (fetchElement(str_Arr).isDisplayed() && fetchElement(str_Arr).isEnabled()) {
                    LogClass.info(str_Element + " is present");
                    return true;
                } else {
                    LogClass.info(str_Element + " is not present or disabled");
                }
            } else {
                LogClass.info(str_Element + " is not present or disabled");
            }
        } catch (Exception e) {
            LogClass.error("Error while checking presence of " + str_Element);
            LogClass.error("The error message is:\n" + e.getMessage());
        }
        return false;
    }
    public static boolean isPresent(String arg1, By by) {
        try {
            ...
            if (Base.driver.findElement(by).isEnabled()) {
                LogClass.info(arg1 + " is present");
                return true;
            } else {
                LogClass.info(arg1 + " is not present");
            }
        } catch (Exception e) {
            LogClass.error("Error while checking presence of " + arg1);
            LogClass.error("The error message is:\n" + e.getMessage());
        }
        return false;
    }
        ...
        try {
            ...
            if (fetchElements(str_Arr).size() > 0) {
                if (fetchElement(str_Arr).isEnabled()) {
                    LogClass.info(str_Element + " is Enabled");
                    return true;
                } else {
                    LogClass.info(str_Element + " is not present or disabled");
                }
            ...
            LogClass.error("Error while checking Enable of " + str_Element);
            LogClass.error("The error message is:\n" + e.getMessage());
        }
        return false;
    }
...
//                obj_SeleniumMethods.performSleep(3);
    ...
    public static boolean click(String str_Element, By by) {
        try {
            ...
            LogClass.info("Clicked on " + str_Element);
            return true;
        } catch (Exception e) {
            isPresent(str_Element, by);
            LogClass.error("Error while clicking on " + str_Element);
            LogClass.error("The error message is:\n" + e.getMessage());
        }
        return false;
    }
